...
uses
  SysUtils, Types,  Classes, Utils;     //Windows,
...
function FloatToStrAE(v: Extended): RawByteString; //overload;
begin
  Str(v, Result);
  {$ifdef Unicode}
...
function FloatToStrAD(v: Double): RawByteString; //overload;
begin
  Str(v, Result);
  {$ifdef Unicode}
  SetCodePage(Result, CP_UTF8, False);
...
function FloatToStrAS(v: Single): RawByteString; //overload;
begin
  Str(v, Result);
  {$ifdef Unicode}
  SetCodePage(Result, CP_UTF8, False);
...
begin
  case TVarData(v).VType of
    ...
    varSingle:   Result := FloatToStrAS(TVarData(v).VSingle);
    varDouble:   Result := FloatToStrAD(TVarData(v).VDouble);
    varCurrency: Result := FloatToStrAE(TVarData(v).VCurrency);
    ...
    varArray + varByte:
      ...
      Result := AnsiToUtf8(WideString(v));
  end;
end;
